...
const makeMessagesSocket = (config) => {
    ...
    const Hisoka = new connect(Utils_1, waUploadToServer, relayMessage);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = Hisoka.detectType(content);
            ...
            if (messageType) {
                switch(messageType) {
                    case 'PAYMENT':
                        const paymentContent = await Hisoka.handlePayment(content, quoted);
                    ...
                    case 'PRODUCT':
                        const productContent = await Hisoka.handleProduct(content, jid, quoted);
                    ...
                    case 'INTERACTIVE':
                        const interactiveContent = await Hisoka.handleInteractive(content, jid, quoted);
                    ...
                    case 'ALBUM':
                        return await Hisoka.handleAlbum(content, jid, quoted)
                    case 'EVENT':
                        return await Hisoka.handleEvent(content, jid, quoted)
                    case 'POLL_RESULT':
                        return await Hisoka.handlePollResult(content, jid, quoted)
                    case 'GROUP_STORY':
                        return await Hisoka.handleGroupStory(content, jid, quoted)
                }
            }
